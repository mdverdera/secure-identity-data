...
public sealed class GetProfileQueryHandlerTests
{
    ...
    private GetProfileQueryHandler CreateHandler() =>
        new(_userRepo.Object, _auditRepo.Object, _currentUser.Object,
            NullLogger<GetProfileQueryHandler>.Instance);

    [Fact]
    public async Task Handle_WhenUserFound_ReturnsProfileDto()
    {
        var user = new User
        {
            Id = Guid.NewGuid(),
        ...
        var result = await handler.Handle(new GetProfileQuery(), default);
    ...
    public async Task Handle_WhenUserNotFound_ThrowsUserNotFoundException()
    {
        ...
        var handler = CreateHandler();
        var act = async () => await handler.Handle(new GetProfileQuery(), default);

        await act.Should().ThrowAsync<UserNotFoundException>()
            .WithMessage("*unknown-subject*");
    }

    [Fact]
    public async Task Handle_WhenUserFound_AppendsAuditLog()
    {
        ...
        var handler = CreateHandler();
        await handler.Handle(new GetProfileQuery(), default);

        _auditRepo.Verify(r => r.AppendAsync(
            It.Is<AuditLog>(log =>
                log.UserId == userId &&
                log.EventType == "ProfileAccessed" &&
                log.Resource == "/api/profile"),
            default), Times.Once);
    }
}
